...
const PLAYER_JUMP_VELOCITY = -530; // Increased jump height
...
export default class GameScene extends Phaser.Scene {
  ...
  private blazeExitTipShown = false;                // Show combat tip once after Blaze zone
  private frostExitTipShown = false;                // Show combat basics tip once after Frost zone
  private orangeDemonTipShown = false;              // Show orange demon tip once on first encounter
  // Non-hook combat gating — persists through death (not reset in create())
  private nonHookRegionCleared: boolean[] = [false, false, false, false, false];
  private combatHintCooldown: number[] = [0, 0, 0, 0, 0]; // time.now of last hint per zone
  ...
  private hookZoneEnemyCleanupDone: boolean[] = [false, false, false, false, false];
  private hookZoneCompletionRespawnAnchor?: { zoneIndex: number; x: number; y: number };
  ...
  create() {
    ...
    this.snowballs = [];
    this.hookZoneEnemyCleanupDone = new Array(this.hookZoneDefinitions.length).fill(false);
    this.hookZoneCompletionRespawnAnchor = undefined;
  ...
  private buildManualPlatforms() {
    ...
    const defs = [
      ...
      { x: 190, y: bandY - 25, width: 240, height: 18 },   // Low step platform (yellow placeholder location)
      { x: 220, y: bandY - 110, width: 160, height: 18 },  // Platform near player start
  ...
  private addDemon(x: number, y: number) {
    ...
    const demonData = {
      ...
      isStunned: false,
      stunDuration: 0,
      progressRegionIndex: this.getProgressRegionIndexForX(x),
    };
  ...
  private getPostBarrierRespawnAnchor(zoneIndex: number): { x: number; y: number } | undefined {
    const zone = this.hookZoneDefinitions[zoneIndex];
    if (!zone) return undefined;

    const nextZoneStart = zoneIndex + 1 < this.hookZoneDefinitions.length
      ? this.hookZoneDefinitions[zoneIndex + 1].start
      : Number.POSITIVE_INFINITY;
    const preferredX = zone.end + 70;

    let bestCandidate: { x: number; y: number; score: number } | undefined;
    ...
    this.physics.world.staticBodies.entries.forEach((entry) => {
      ...
      const centerX = left + staticBody.width / 2;

      if (centerX <= zone.end + 8) return;
      if (centerX >= nextZoneStart - 8) return;

      const anchorX = Phaser.Math.Clamp(preferredX, left + 10, right - 10);
      const anchorY = staticBody.y - 60;
      const score = Math.abs(centerX - preferredX) + Math.abs(anchorY - (this.spawnPoint.y - 20)) * 0.15;

      if (!bestCandidate || score < bestCandidate.score) {
        bestCandidate = { x: anchorX, y: anchorY, score };
      }
    });
    ...
    const groundY = this.map.heightInPixels - 16;
    return { x: zone.end + 70, y: groundY - 60 };
  }

  private setHookZoneCompletionRespawnAnchor(zoneIndex: number) {
    const anchor = this.getPostBarrierRespawnAnchor(zoneIndex);
    if (!anchor) return;

    this.hookZoneCompletionRespawnAnchor = {
      zoneIndex,
      x: anchor.x,
      y: anchor.y,
    };
  ...
  private getCompletedHookZoneExitRespawnPointAtPlayerPosition():
    ...
    const completionAnchor = this.hookZoneCompletionRespawnAnchor;
    if (completionAnchor && this.hookZoneSolved[completionAnchor.zoneIndex]) {
      return {
        zoneIndex: completionAnchor.zoneIndex,
  ...
  private respawnOutsideHookZoneEntranceIfNeeded(): boolean {
    ...
    if (this.hookZoneSolved[zoneIndex]) return false;
  ...
  private showHookTutorial() {
    ...
    const timer = this.add.text(centerX, centerY + 155, '20', {
      fontFamily: 'Arial, sans-serif',
      fontSize: '24px',
      color: '#60a5fa',
      stroke: '#000000',
    ...
    let countdownInterval: Phaser.Time.TimerEvent | undefined;
    ...
    // Countdown animation (20 seconds total)
    countdownInterval = this.time.addEvent({
      delay: 1000,
      repeat: 19, // 20 iterations
      callback: () => {
        countdown--;
    ...
    // Allow player to close early with click
    bg.setInteractive();
  ...
  private handleVoiceCommand(spokenWord: string) {
    ...
    if (this.waitingForFrostPronunciation) {
      if (normalizedSpoken === 'FROST' || normalizedSpoken.includes('FROST')) {
        ...
        this.launchHugeFrostAttack(true);
        return;
      } else {
  ...
  private checkHookZoneStatus() {
    ...
    if (inZone && !this.isInHookZone) {
      // Gate: all enemies in the preceding non-hook region must be cleared first
      if (!this.isNonHookRegionClearedForZone(zoneIndex)) {
        this.blockZoneEntry(zoneIndex);
        this.maybeShowCombatBlockHint(zoneIndex);
        return;
      }
    ...
    // Player trying to exit a hook zone
      ...
      if (this.currentHookZoneIndex >= 0 && !this.hookZoneSolved[this.currentHookZoneIndex]) {
        ...
        const exitingZoneIdx = this.currentHookZoneIndex;
        this.exitHookZone();
        if (exitingZoneIdx === 0 && !this.blazeExitTipShown) {
          this.blazeExitTipShown = true;
          this.showBlazeExitTip();
        }
        if (exitingZoneIdx === 1 && !this.frostExitTipShown) {
          this.frostExitTipShown = true;
          this.showFrostExitTip();
        }
      }
    }
  }
  ...
  private enterHookZone(zoneIndex: number) {
    ...
    this.cleanupPreviousRegionEnemiesOnHookEntry(zoneIndex);
  ...
  private showBlazeExitTip() {
    const cx = this.cameras.main.width / 2;
    const cy = 82;
    const B = 4; // pixel block size for flame art

    // Panel background
    const bg = this.add.rectangle(cx, cy, 450, 148, 0x0a0a1e, 0.92);
    bg.setStrokeStyle(2, 0xff7700);
    bg.setScrollFactor(0);
    bg.setDepth(2100);

    // Title
    const title = this.add.text(cx + 16, cy - 44, 'SPEECH SPELLS', {
      fontFamily: 'monospace',
      fontSize: '15px',
      color: '#fbbf24',
      fontStyle: 'bold',
    ...
    // Body
    const body = this.add.text(cx + 16, cy + 8,
      'Press V to cast spells by voice\nBlue demons throw ice and slow you\nSpeak  BLAZE  to defeat them', {
      fontFamily: 'monospace',
      fontSize: '12px',
      color: '#e8e8e8',
      stroke: '#000000',
    ...
    // Pixel-art flame icon (left side of panel)
    const g = this.add.graphics();
    g.setScrollFactor(0);
    g.setDepth(2101);
    const fx = cx - 198;
    ...
    g.fillRect(fx - B,     fy + B * 2, B * 3, B * 2);
    // Orange mid
    g.fillStyle(0xff8800, 1);
    g.fillRect(fx - B,     fy - B,     B * 3, B * 3);
    g.fillRect(fx - B * 2, fy,         B,     B * 2);
    g.fillRect(fx + B * 2, fy - B,     B,     B * 2);
    // Red tip
    g.fillStyle(0xff2200, 1);
    g.fillRect(fx,         fy - B * 4, B,     B * 3);
    g.fillRect(fx - B,     fy - B * 3, B,     B);
    g.fillRect(fx + B,     fy - B * 3, B,     B);

    const elements: Phaser.GameObjects.GameObject[] = [bg, title, body, g];

    // Auto-dismiss: fade out after 15 s
    this.time.delayedCall(15000, () => {
      this.tweens.add({
        targets: elements,
        alpha: 0,
        duration: 700,
        onComplete: () => elements.forEach(o => o.destroy()),
      });
    });
  }

  private showFrostExitTip() {
    const cx = this.cameras.main.width / 2;
    const cy = 90;
    const B = 3; // pixel block size for sprites

    // Panel background (blue-tinted border to match frost theme)
    const bg = this.add.rectangle(cx, cy, 460, 165, 0x0a0a1e, 0.92);
    bg.setStrokeStyle(2, 0x4499ff);
    bg.setScrollFactor(0);
    bg.setDepth(2100);

    // Title
    const title = this.add.text(cx + 16, cy - 56, 'COMBAT BASICS', {
      fontFamily: 'monospace',
      fontSize: '15px',
      color: '#fbbf24',
      fontStyle: 'bold',
    ...
    // Body
    const body = this.add.text(cx + 16, cy + 10,
      'Press J to switch WAND / SWORD\nWAND  \u2014  Press E to shoot magic bolts\nSWORD \u2014  Press Q for sword barrage', {
      fontFamily: 'monospace',
      fontSize: '12px',
      color: '#e8e8e8',
      stroke: '#000000',
    ...
    // Pixel-art wand & sword icons (left side of panel)
    const g = this.add.graphics();
    g.setScrollFactor(0);
    g.setDepth(2101);

    // --- Wand sprite (upper icon) ---
    const wx = cx - 202;
    const wy = cy - 8;
    // Star tip (yellow cross)
    g.fillStyle(0xffee44, 1);
    g.fillRect(wx,         wy - B * 2, B, B * 3); // vertical arm
    g.fillRect(wx - B,     wy - B,     B * 3, B); // horizontal arm
    // Shaft (brown)
    g.fillStyle(0x8b5a14, 1);
    g.fillRect(wx,         wy + B,     B, B * 4); // shaft downward

    // --- Sword sprite (lower icon) ---
    const sx = cx - 204;
    const sy = cy + 28;
    // Blade (silver, horizontal pointing right)
    g.fillStyle(0xd0d0d0, 1);
    g.fillRect(sx,         sy,         B * 5, B);   // blade body
    g.fillRect(sx + B * 5, sy - B,     B,     B);   // pointed tip
    // Crossguard (gold, vertical)
    g.fillStyle(0xc8922a, 1);
    g.fillRect(sx - B,     sy - B * 2, B, B * 5);   // guard bar
    // Handle (dark brown)
    g.fillStyle(0x5a2a0a, 1);
    g.fillRect(sx - B * 3, sy - B,     B * 2, B * 3); // grip
  ...
  private showOrangeDemonTip() {
    const cx = this.cameras.main.width / 2;
    const cy = 110;
    const B = 3; // pixel block size for sprites

    // Panel background (orange border for fire theme)
    const bg = this.add.rectangle(cx, cy, 490, 208, 0x0a0a1e, 0.92);
    bg.setStrokeStyle(2, 0xff6600);
    bg.setScrollFactor(0);
    bg.setDepth(2100);

    // Title
    const title = this.add.text(cx + 16, cy - 75, 'ORANGE DEMONS', {
      fontFamily: 'monospace',
      fontSize: '15px',
      color: '#fbbf24',
      fontStyle: 'bold',
    ...
    // Body text
    const body = this.add.text(cx + 16, cy + 12,
      'Beam hits you 3+ sec = death\nSpeak  FROST  to freeze + disable beam\nFrozen: J=swap  SWORD(Q barrage)  WAND(E bolts)\nStorm, Hungry + other spells also work\nBlaze does NOT work \u2014 hold V+move to cast', {
      fontFamily: 'monospace',
      fontSize: '12px',
      color: '#e8e8e8',
      stroke: '#000000',
    ...
    // Pixel-art sprites (left side of panel)
    const g = this.add.graphics();
    g.setScrollFactor(0);
    g.setDepth(2101);

    // --- Active demon sprite (upper left) ---
    const ax = cx - 210;
    const ay = cy - 32;

    // Horns (dark red)
    g.fillStyle(0xcc2200, 1);
    g.fillRect(ax - B,     ay - B * 6, B, B * 2);
    g.fillRect(ax + B * 2, ay - B * 6, B, B * 2);
    // Head (bright orange)
    g.fillStyle(0xff6600, 1);
    g.fillRect(ax - B, ay - B * 4, B * 4, B * 3);
    // Eyes (yellow)
    g.fillStyle(0xffdd00, 1);
    g.fillRect(ax - B + 2, ay - B * 3 + 2, 2, 2);
    g.fillRect(ax + B + 2, ay - B * 3 + 2, 2, 2);
    // Body
    g.fillStyle(0xff5500, 1);
    g.fillRect(ax, ay, B * 2, B * 3);
    // Legs
    g.fillStyle(0xff4400, 1);
    g.fillRect(ax,     ay + B * 3, B, B * 2);
    g.fillRect(ax + B, ay + B * 3, B, B * 2);
    // Beam (small orange line shooting right)
    g.fillStyle(0xffaa00, 1);
    g.fillRect(ax + B * 3,     ay - B + 2, B * 5, 2);
    g.fillRect(ax + B * 4, ay - B,     B * 3, 2);

    // "ACTIVE" label below sprite
    const activeLabel = this.add.text(ax, ay + B * 6 + 2, 'ACTIVE', {
      fontFamily: 'monospace',
      fontSize: '9px',
      color: '#ff8800',
      stroke: '#000000',
    ...
    activeLabel.setOrigin(0.5, 0);
    activeLabel.setScrollFactor(0);
    activeLabel.setDepth(2101);

    // --- Frozen demon sprite (lower left) ---
    const fx = cx - 210;
    const fy = cy + 44;

    // Horns (dark blue)
    g.fillStyle(0x2255aa, 1);
    g.fillRect(fx - B,     fy - B * 6, B, B * 2);
    g.fillRect(fx + B * 2, fy - B * 6, B, B * 2);
    // Head (light blue)
    g.fillStyle(0x4499dd, 1);
    g.fillRect(fx - B, fy - B * 4, B * 4, B * 3);
    // Eyes (cyan)
    g.fillStyle(0xaaeeff, 1);
    g.fillRect(fx - B + 2, fy - B * 3 + 2, 2, 2);
    g.fillRect(fx + B + 2, fy - B * 3 + 2, 2, 2);
    // Body (medium blue)
    g.fillStyle(0x3377cc, 1);
    g.fillRect(fx, fy, B * 2, B * 3);
    // Legs
    g.fillStyle(0x2266aa, 1);
    g.fillRect(fx,     fy + B * 3, B, B * 2);
    g.fillRect(fx + B, fy + B * 3, B, B * 2);
    // Frost crystals (white cross overlay)
    g.fillStyle(0xffffff, 1);
    g.fillRect(fx - B * 2, fy - B, B * 7, 2);       // horizontal bar
    g.fillRect(fx + B - 1, fy - B * 4, 2, B * 8);   // vertical bar
    // Corner frost dots
    g.fillRect(fx - B * 2 - 2, fy - B * 3, 2, 2);
    g.fillRect(fx + B * 4 + 2, fy - B * 3, 2, 2);
    g.fillRect(fx - B * 2 - 2, fy + B * 3, 2, 2);
    g.fillRect(fx + B * 4 + 2, fy + B * 3, 2, 2);

    // "FROZEN" label below sprite
    const frozenLabel = this.add.text(fx, fy + B * 6 + 2, 'FROZEN', {
      fontFamily: 'monospace',
      fontSize: '9px',
      color: '#88ccff',
      stroke: '#000000',
    ...
    frozenLabel.setOrigin(0.5, 0);
    frozenLabel.setScrollFactor(0);
    frozenLabel.setDepth(2101);

    const elements: Phaser.GameObjects.GameObject[] = [bg, title, body, g, activeLabel, frozenLabel];
  ...
  /**
   * Returns true when every enemy in the non-hook region preceding zoneIndex is dead.
   * A region with zero enemies is considered cleared immediately.
   * Result is cached in nonHookRegionCleared[] (persists through death).
   */
  private isNonHookRegionClearedForZone(zoneIndex: number): boolean {
    if (this.nonHookRegionCleared[zoneIndex]) return true;
    const alive = this.enemies.filter(e => e.progressRegionIndex === zoneIndex).length;
    if (alive === 0) {
      this.nonHookRegionCleared[zoneIndex] = true;
      return true;
    }
    return false;
  }
  ...
  /**
   * Push the player back outside the zone entrance (mirrors blockZoneExit logic).
   */
  private blockZoneEntry(zoneIndex: number) {
    const zone = this.hookZoneDefinitions[zoneIndex];
    const body = this.player.body as Phaser.Physics.Arcade.Body;
    if (!body) return;
    this.player.x = zone.start - 20;
  ...
  /**
   * Show the "Defeat all enemies" hint at most once every 4 seconds per zone.
   */
  private maybeShowCombatBlockHint(zoneIndex: number) {
    const now = this.time.now;
    if (now - this.combatHintCooldown[zoneIndex] < 4000) return;
    this.combatHintCooldown[zoneIndex] = now;

    const cx = this.cameras.main.width / 2;
    const cy = this.cameras.main.height - 58;

    const bg = this.add.rectangle(cx, cy, 330, 38, 0x0a0a1e, 0.9);
    bg.setStrokeStyle(1, 0xff4444);
    bg.setScrollFactor(0);
    bg.setDepth(2200);

    const txt = this.add.text(cx, cy, 'Defeat all enemies to proceed.', {
      fontFamily: 'monospace',
      fontSize: '13px',
      color: '#ff9999',
      stroke: '#000000',
  ...
  private checkHookZoneCompletion() {
...

    {
      ...
      // Mark zone as solved
      this.hookZoneSolved[this.currentHookZoneIndex] = true;
      this.setHookZoneCompletionRespawnAnchor(this.currentHookZoneIndex);
  ...
  private updateCombat(delta: number) {
    ...
    // Orange demon contextual tip — trigger once when player approaches first fire demon area
    if (this.fireDemonsSpawned && !this.orangeDemonTipShown && this.player.x > 4000) {
      this.orangeDemonTipShown = true;
      this.showOrangeDemonTip();
    }
  ...
  private launchHugeFrostAttack(fromSpokenFrost: boolean = false) {
    ...
    this.enemies.forEach(enemy => {
      if (!enemy.isDead && !enemy.sprite.getData('isFrozen')) {
        ...
        if (distance <= 1500) {
          if (enemy.enemyType === 'fire') {
            this.freezeFireDemon(enemy, 15000, fromSpokenFrost ? 'SPOKEN_FROST' : 'OTHER');
            return;
          }
  ...
  private launchFrostBlizzardStun() {
    ...
    this.enemies.forEach(enemy => {
      if (!enemy.isDead && !enemy.sprite.getData('isFrozen')) {
        if (enemy.enemyType === 'fire') {
          this.freezeFireDemon(enemy, 15000, 'SPOKEN_FROST');
          return;
        }
  ...
  private launchFlareStun() {
    ...
    this.enemies.forEach(enemy => {
      if (!enemy.isDead) {
        ...
        // Damage enemy (scorch damage) — tagged BLAZE so fire demons remain immune
        this.damageEnemy(enemy, 3, 'BLAZE');
      }
    });
  ...
  private getProgressRegionIndexForX(x: number): number {
    for (let i = 0; i < this.hookZoneDefinitions.length; i++) {
      if (x < this.hookZoneDefinitions[i].start) {
        return i;
      }
    }
    return this.hookZoneDefinitions.length;
  }

  private forceDestroyEnemyForProgressCleanup(enemy: any) {
  ...
  private cleanupPreviousRegionEnemiesOnHookEntry(zoneIndex: number) {
    if (zoneIndex < 0 || zoneIndex >= this.hookZoneDefinitions.length) return;
    if (this.hookZoneEnemyCleanupDone[zoneIndex]) return;

    this.hookZoneEnemyCleanupDone[zoneIndex] = true;

    for (let i = this.demons.length - 1; i >= 0; i--) {
      const demon = this.demons[i];
      if (demon.progressRegionIndex <= zoneIndex) {
        this.killDemon(demon);
      }
    }
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (!enemy.isDead && enemy.progressRegionIndex <= zoneIndex) {
        this.forceDestroyEnemyForProgressCleanup(enemy);
      }
    }
  }
  ...
  private freezeFireDemon(enemy: any, durationMs: number, source: 'SPOKEN_FROST' | 'OTHER' = 'OTHER') {
    if (!enemy || enemy.isDead || enemy.enemyType !== 'fire') return;
    if (source !== 'SPOKEN_FROST') return;
    ...
    this.fireBeamBurnMeter = 0; // Reset exposure timer immediately on freeze
  }
  ...
  private canEnemyBeDefeatedBy(enemy: any, source: string): boolean {
    ...
    if (enemy.enemyType === 'fire') {
      if (source === 'BLAZE' || source === 'BLAZE_SPEECH') return false; // Blaze never damages fire demons
      return enemy.fireState === 'FROZEN'; // All other attacks only work while frost-disabled
    }
  ...
  private handlePlayerDeath() {
    ...
    if (this.lives <= 0) {
      console.log('💀 Out of lives! Resetting lives and respawning at checkpoint.');
      this.lives = 3;
    }
